                        ...
                        <ScrollArea className="flex-1 p-6">
                            <div className="space-y-6 px-1">
                                ...
                                <CardSection title="Emotional Expression" icon={Zap} color="orange" fullWidth>
                                    <div className="grid grid-cols-3 gap-3">
                                        <MiniSelect label="Logos" value={selectedCharacter.emotional?.logos} onChange={(v: string) => onUpdate(selectedCharacter.id, updateNested(selectedCharacter, ['emotional', 'logos'], v))} options={LOGOS_OPTIONS} tooltip="Cara karakter menggunakan logika & penalaran dalam berkomunikasi" />
                                        <MiniSelect label="Ethos" value={selectedCharacter.emotional?.ethos} onChange={(v: string) => onUpdate(selectedCharacter.id, updateNested(selectedCharacter, ['emotional', 'ethos'], v))} options={ETHOS_OPTIONS} tooltip="Kredibilitas & karakter moral yang ditampilkan saat berbicara" />
                                        <MiniSelect label="Pathos" value={selectedCharacter.emotional?.pathos} onChange={(v: string) => onUpdate(selectedCharacter.id, updateNested(selectedCharacter, ['emotional', 'pathos'], v))} options={PATHOS_OPTIONS} tooltip="Cara karakter mengekspresikan & mengelola emosi" />
                                    </div>
                                    <div className="grid grid-cols-3 gap-3">
                                        <MiniSelect label="Tone" value={selectedCharacter.emotional?.tone} onChange={(v: string) => onUpdate(selectedCharacter.id, updateNested(selectedCharacter, ['emotional', 'tone'], v))} options={EXPRESSION_TONE_OPTIONS} tooltip="Nada suara & attitude saat berkomunikasi" />
                                        <MiniSelect label="Style" value={selectedCharacter.emotional?.style} onChange={(v: string) => onUpdate(selectedCharacter.id, updateNested(selectedCharacter, ['emotional', 'style'], v))} options={EXPRESSION_STYLE_OPTIONS} tooltip="Gaya penyampaian pesan (langsung/tidak langsung)" />
                                        <MiniSelect label="Mode" value={selectedCharacter.emotional?.mode} onChange={(v: string) => onUpdate(selectedCharacter.id, updateNested(selectedCharacter, ['emotional', 'mode'], v))} options={EXPRESSION_MODE_OPTIONS} tooltip="Cara berinteraksi dengan orang lain" />
                                    </div>
                                </CardSection>
                                ...
                                <CardSection title="Trustworthy" icon={Shield} color="teal" fullWidth>
                                    <div className="grid grid-cols-3 gap-3">
                                        <MiniSelect label="Willingness" value={selectedCharacter.coreBeliefs?.willingness} onChange={(v: string) => onUpdate(selectedCharacter.id, updateNested(selectedCharacter, ['coreBeliefs', 'willingness'], v))} options={WILLINGNESS_OPTIONS} tooltip="Kesediaan karakter untuk membantu & berkomitmen" />
                                        <MiniSelect label="Vulnerability" value={selectedCharacter.coreBeliefs?.vulnerability} onChange={(v: string) => onUpdate(selectedCharacter.id, updateNested(selectedCharacter, ['coreBeliefs', 'vulnerability'], v))} options={VULNERABILITY_OPTIONS} tooltip="Keterbukaan karakter terhadap kelemahan diri" />
                                        <MiniSelect label="Integrity" value={selectedCharacter.coreBeliefs?.integrity} onChange={(v: string) => onUpdate(selectedCharacter.id, updateNested(selectedCharacter, ['coreBeliefs', 'integrity'], v))} options={INTEGRITY_OPTIONS} tooltip="Konsistensi antara nilai & tindakan karakter" />
                                    </div>
