...
interface IPPassportProps {
    ...
    onDeleteCoverVersion?: (versionId: string) => void;
    onRestoreCoverVersion?: (versionId: string) => void;
    deletedCoverVersions?: CoverVersion[];
}
...
export function IPPassport({
    ...
    onDeleteCoverVersion, onRestoreCoverVersion, deletedCoverVersions = []
}: IPPassportProps) {
                        ...
                        <div className="mt-3 relative">
                            ...
                            {showCoverDropdown && (
                                <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-xl border border-slate-200 z-50 overflow-hidden">
                                            ...
                                            coverVersions
                                                ...
                                                .map((version) => (
                                                    ...
                                                    >
                                                        ...
                                                        {/* Delete Button */}
                                                        {!version.isActive && onDeleteCoverVersion && (
                                                            <button
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                                    if (confirm('Delete this cover version?')) {
                                                                        onDeleteCoverVersion(version.id);
                                                                    }
                                                                }}
                                    ...
                                    {deletedCoverVersions.length > 0 && (
                                        <div className="border-t border-slate-100">
                                            <div className="px-3 py-2 bg-red-50/50">
                                                <span className="text-xs font-medium text-red-600">🗑️ Deleted ({deletedCoverVersions.length})</span>
                                            </div>
                                            <div className="max-h-32 overflow-y-auto">
                                                {deletedCoverVersions.map((version) => (
                                                    <div
                                                        key={version.id}
                                                        className="p-3 flex items-center gap-3 bg-red-50/30 hover:bg-red-50 transition-colors"
                                                    >
                                                        {/* Thumbnail */}
                                                        <div className="w-10 h-14 rounded-lg overflow-hidden bg-slate-100 flex-shrink-0 opacity-60">
                                                            <img
                                                                src={version.imageUrl}
                                                                alt={`Cover v${version.versionNumber}`}
                                                                className="w-full h-full object-cover"
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            ...
                                                            <p className="text-[10px] text-red-400">
                                                                Deleted {version.deletedAt ? new Date(version.deletedAt).toLocaleDateString() : ''}
                                                            </p>
                                                        </div>

                                                        {/* Restore Button */}
                                                        {onRestoreCoverVersion && (
                                                            <button
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                                    onRestoreCoverVersion(version.id);
                                                                }}
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    ...
                                    <Input
                                        ...
                                        className={`pl-10 bg-white/50 border-slate-200 focus:border-orange-500 transition-all ${existingProtagonist ? 'opacity-60 cursor-not-allowed bg-slate-100' : ''}`}
                                        placeholder="e.g. John Doe, Andi Pratama..."
                                        disabled={!!existingProtagonist}
                                        readOnly={!!existingProtagonist}
                                    />
                                </div>
